    ...
    <div id="kmt_admin_middle_right">
        ...
        <div id="kmt_admin_content">
                ...
                <table class="kmt_admin_table">
                    ...
                    <?php 
                     ...
                     foreach ($menu as $value){
                        $url = base_url().'same_method/udp_status/kmt_menu_2/'.$value->idMN_2.'/';
                    ?>
                    <input type="hidden" name="id[]" value="<?=$value->idMN_2?>"/>
                    <tr <?php if($i%2==0){echo "class='kmt_admin_c1'";}else{echo "class='kmt_admin_c2'";}?>>
                        <td><input class="item" type="checkbox" value="<?=$value->idMN_2?>" name="id_check[]"/></td>
                        <td><a href="<?=base_url()?>admin_menu/edit_menu_2/<?=$value->idMN_2?>"><img src="<?=base_url()?>public/admin/img/b_edit.png"/></a></td>
                        <td><input type="text" value="<?=$value->STT?>" name="stt[]"/></td>
                        <td>
                           <?php if($value->TrangThai==1){?>
                             <a href="javascript:void(0)" onclick="udp_status('<?=$url?>','idMN_2','TrangThai', 0)">
                                <img src="<?=base_url()?>public/admin/img/on.gif"/>
                             </a>
                           <?php }else{?>
                             <a href="javascript:void(0)" onclick="udp_status('<?=$url?>','idMN_2','TrangThai', 1)">
                        ...
                        <td class="kmt_hidden">
                           <?php if($value->TieuBieu==1){?>
                             <a href="javascript:void(0)" onclick="udp_status('<?=$url?>','idMN_2','TieuBieu', 0)">
                                <img src="<?=base_url()?>public/admin/img/on.gif"/>
                             </a>
                           <?php }else{?>
                             <a href="javascript:void(0)" onclick="udp_status('<?=$url?>','idMN_2','TieuBieu', 1)">
                                <img src="<?=base_url()?>public/admin/img/off.gif"/>
                             </a>
                           <?php }?>
                        </td>
                        <td><a onclick="return confirm('Bạn có chắc muốn xóa các danh mục đã chọn, nếu vẫn còn bài viết nào thuộc danh mục này bạn sẽ không được xóa nó!')" href="<?=base_url()?>same_method/del_one_check/kmt_menu_2/<?=$value->idMN_2?>"><img src="<?=base_url()?>public/admin/img/b_drop.png"/></a></td>
            ...
            <div class="kmt_admin_bar_3">
                <p>
                    <a href="<?=base_url()?>admin_menu/add_menu_2"><input type="button" value="Thêm mới"/></a>
                ...
                <p>
                    ...
                    <input type="button" value="Lọc" class="button" onclick="window.location='<?=base_url()?>admin_menu/menu_2_filter/'+document.getElementById('idMN_1_filter').value"/>
                </p>
            </div>
            <!-- End kmt_admin_bar_3-->
...
<script type="text/javascript">
	   $('#bt_ud').click(function() {
	        var url = "<?=base_url()?>same_method/ud_numbers/kmt_menu_2/idMN_2";
       ...
       $('#bt_del').click(function() {
	        var url = "<?=base_url()?>same_method/del_all_check/kmt_menu_2";
       ...
       function changeForm() {
           ...
           var url = "<?=base_url()?>same_method/upp_status_list/kmt_menu_2/idMN_2/1";
           if(selectedValue=='0'){
        	   return false;
           }else{
               //begin Swich
               switch (selectedValue) { 
                    case '1': 
                        ...
                        break;
                    case '2': 
                        url = "<?=base_url()?>same_method/upp_status_list/kmt_menu_2/idMN_2/2";
                        $("#form").attr("action", url);
                        $('#form').submit();
                        break;
                    case '3': 
                        url = "<?=base_url()?>same_method/upp_status_list/kmt_menu_2/idMN_2/3";
                        $("#form").attr("action", url);
                        $('#form').submit();
                        break;      
                    case '4': 
                        url = "<?=base_url()?>same_method/upp_status_list/kmt_menu_2/idMN_2/4";
                        $("#form").attr("action", url);
                        $('#form').submit();
                        break;
                    default:
